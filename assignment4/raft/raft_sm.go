...
func (sm *StateMachine) Alarm(timertype int) {
	if timertype == 1 {
		...
		(*sm.RaftNode).Timer = time.NewTimer(time.Millisecond*time.Duration((*sm.RaftNode).ElectionTimeout))
	} else if timertype == 3 {
		(*sm.RaftNode).Timer = time.NewTimer(time.Millisecond*time.Duration((*sm.RaftNode).HeartbeatTimeout))
	} else if timertype == 4 {
		(*sm.RaftNode).Timer = time.NewTimer(time.Millisecond*time.Duration((*sm.RaftNode).ElectionTimeout))
...
func (sm *StateMachine) LogDelete(index int) {
	...
	for _, log := range sm.Log[index:] {
		if log.Data != nil && log.State == 1 {
				(*sm.RaftNode).CommitChannel <- CommitInfo{Data:log.Data, Index:-1, Err:errors.New("I am a FolloweR")}
		}
	}
	sm.Log=sm.Log[:index]
...
func (sm *StateMachine) toLeader() []SendEvent {
	...
	sm.LogStore(sm.LastLogIndex+1,sm.Term, []byte("TL"))
...
func (sm *StateMachine) handleTimeoutEvent(ev Timeout) {
	...
	switch sm.State {
		...
		sm.Alarm(3)
				...
				sm.LogStore(sm.LastLogIndex+1,sm.Term, []byte("TO"))
				for _,id := range sm.ServerIds {
					if id != sm.Id {
						sendEvent = sm.Send(id, AppendEntriesReq{sm.Term, sm.Id, sm.NextIndex[id]-1, sm.Log[sm.NextIndex[id]-1].Term,sm.Log[sm.NextIndex[id]:], sm.CommitIndex})
						sendEvents = append(sendEvents,sendEvent)
...
func (sm *StateMachine) Commit (beginIndex int, endIndex int) {
	if sm.is_not_sm_test !=0 {
		...
		(*sm.RaftNode).CommitChannel <- CommitInfo{Data:sm.Log[endIndex].Data, Index:int64(endIndex), Err:nil}
	}
}
